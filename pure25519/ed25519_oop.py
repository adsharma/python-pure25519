...
class SigningKey(object):
    ...
    def __eq__(self, them):
        if not isinstance(them, object):
            return False
        return them.__class__ == self.__class__ and them.sk_s == self.sk_s
...
class VerifyingKey(object):
    ...
    def __eq__(self, them):
        if not isinstance(them, object):
            return False
        return them.__class__ == self.__class__ and them.vk_s == self.vk_s
...
def selftest():
    ...
    sk = SigningKey(
        b"priv0-VIsfn5OFGa09Un2MR6Hm7BQ5++xhcQskU2OGXG8jSJl4cWLZrRrVcSN2gVYMGtZT+3354J5jfmqAcuRSD9KIyg",
        prefix="priv0-",
        encoding="base64",
    )
    vk = VerifyingKey(
        b"pub0-eHFi2a0a1XEjdoFWDBrWU/t9+eCeY35qgHLkUg/SiMo",
        prefix="pub0-",
        encoding="base64",
    )
    ...
    assert (
        sig
        == b"sig0-E/QrwtSF52x8+q0l4ahA7eJbRKc777ClKNg217Q0z4fiYMCdmAOI+rTLVkiFhX6k3D+wQQfKdJYMxaTUFfv1DQ"
    ), sig
